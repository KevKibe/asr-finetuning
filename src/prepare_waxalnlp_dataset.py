#!/usr/bin/env python3
"""Build a combined WaxalNLP ASR dataset from raw parquet shards.
...
if len(sys.argv) != 3:
    print("Usage: python prepare_waxalnlp_dataset.py <waxalnlp_root> <combined_root>")
    sys.exit(1)

waxalnlp_root = Path(sys.argv[1])
combined_root = Path(sys.argv[2])
...
if not waxalnlp_root.is_dir():
    raise FileNotFoundError(f"WaxalNLP dataset directory does not exist: {waxalnlp_root}")

source_root = waxalnlp_root / "data" / "ASR"
if not source_root.is_dir():
    # Allow passing the ASR subdirectory directly.
    source_root = waxalnlp_root

shutil.rmtree(combined_root, ignore_errors=True)
combined_root.mkdir(parents=True)

manifest = {
    "waxalnlp_root": str(waxalnlp_root),
    "partitions": [],
}

language_dirs = sorted(p for p in source_root.iterdir() if p.is_dir())
if not language_dirs:
    raise FileNotFoundError(f"No language directories found under: {source_root}")

for language_dir in language_dirs:
    source_language = language_dir.name
    parquet_files = sorted(language_dir.glob("*.parquet"))
    if not parquet_files:
        continue

    grouped: dict[str, list[Path]] = {}
    for parquet_file in parquet_files:
        match = WAXAL_FILE_RE.match(parquet_file.name)
        if not match:
            continue

        split = destination_split(match.group("split"))
        grouped.setdefault(split, []).append(parquet_file)

    for target_split, split_files in sorted(grouped.items()):
        destination_dir = combined_root / "corpus=ASR" / f"split={target_split}" / f"language={source_language}"
...
print(f"Combined WaxalNLP dataset ready at: {combined_root}")
for partition in manifest["partitions"]:
    print(
        f"  {partition['source_language']}:{partition['source_split']} -> "
        f"{partition['destination_split']} ({partition['parquet_files']} parquet file(s), "
        f"dropped_rows={partition['dropped_rows']}, dropped_files={partition['dropped_files']}, "
        f"normalized_audio_paths={partition['normalized_audio_paths']})"
    )